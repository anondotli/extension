...
export function VaultSetupPrompt({ baseUrl, refreshing, onRefresh }: VaultSetupPromptProps) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6 text-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <h2 className="text-base font-semibold tracking-tight">Set up your vault</h2>
        <p className="text-xs text-muted-foreground leading-relaxed max-w-[280px]">
          Your alias labels, notes, and drop keys are end-to-end encrypted. Create a vault password on anon.li to enable encrypted sync across devices.
        </p>
      </div>
      ...
      <div className="w-full max-w-[260px] flex flex-col gap-2">
        <Button
          type="button"
          onClick={() => browser.tabs.create({ url: `${baseUrl}/setup` })}
        >
          Set up vault on anon.li ↗
        </Button>
        <Button
          type="button"
          variant="outline"
          ...
          onClick={onRefresh}
        >
          {refreshing ? "Checking…" : "I've set it up — continue"}
        </Button>
      </div>
    </div>
  );
